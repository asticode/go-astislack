...
func (s *Slack) Delete(query string) (err error) {
	// Loop
	var resp *Response
	for resp == nil || (resp.Files.Pagination.TotalCount > 0 || resp.Messages.Pagination.TotalCount > 0) {
		// Create parameters
		ps := url.Values{}
		ps.Set("query", query)

		// Add page
		// Final files page count is 0 and final message page count is 1
		if resp != nil && ((resp.Files.Pagination.PageCount > 0 && resp.Files.Pagination.Page < resp.Files.Pagination.PageCount) || (resp.Messages.Pagination.PageCount > 1 && resp.Messages.Pagination.Page < resp.Messages.Pagination.PageCount)) {
